...
class BitArticleTopic extends BitBase
{
	...
	function storeTopic($iParamHash = NULL) {
		...
		if ($this->verify($iParamHash)) {
			if (!$iParamHash['topic_id']) {
				$topicId = $this->GenID('tiki_article_topics_topic_id_seq');
			} else {
			...
			if ($iParamHash['topic_id']) {
				$this->associateUpdate(BIT_DB_PREFIX."tiki_article_topics", $iParamHash, array('name' => 'topic_id', 'value'=> $iParamHash['topic_id']));				
			} else {
				$iParamHash['topic_id'] = $topicId;
				$this->associateInsert(BIT_DB_PREFIX."tiki_article_topics", $iParamHash);	
			}			
		}
	...
	function removeTopicImage() {
		if ($this->mTopicId) {
			...
			$rs = $this->query($sql, array($this->mTopicId));
	...
	function removeTopic($iRemoveArticles = FALSE) {
		...
		if ($iRemoveArticles == TRUE) {
			...
			$rs = $this->query($sql, array(NULL, $this->mTopicId));
		}

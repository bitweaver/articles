...
array( 'PHP' => '
	global $gBitSystem;
	require_once( ARTICLE_PKG_PATH."BitArticle.php" );
	require_once( ARTICLE_PKG_PATH."BitArticleTopics.php" );
	require_once( ARTICLE_PKG_PATH."BitArticleTypes.php" );
	...
	if( $rs = $gBitSystem->mDb->query( $query ) ) {
		while( !$rs->EOF ) {
			...
			$rs->fields["content_type_guid"] = ARTICLE_CONTENT_TYPE_GUID;
			$rs->fields["format_guid"] = PLUGIN_GUID_TIKIWIKI;
			$gBitSystem->mDb->associateInsert( "tiki_content", $rs->fields );
			$gBitSystem->mDb->query( "UPDATE `'.BIT_DB_PREFIX.'tiki_articles` SET `content_id`=? WHERE `article_id`=?", array( $conId, $rs->fields["article_id"] ) );

			// store article image
			if( !empty( $rs->fields["image_name"] ) ) {
				$attId = $gBitSystem->mDb->GenID( "tiki_attachments" );
				$tempImagePath = $tempDir.$fileHash["image_name"];
				if( $handle = fopen( $tempImagePath, "a" ) ) {
...
//	$gBitSystem->registerUpgrade( ARTICLE_PKG_NAME, $upgrades[$gUpgradeFrom][$gUpgradeTo] );
//}

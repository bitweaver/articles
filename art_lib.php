...
class ArtLib extends BitBase {
    ...
    function add_article_hit( $articleId ) {
        ...
        if ( $count_admin_pvs == 'y' || $bit_p_admin != 'y' ) {
            ...
            $result = $this->query( $query, array( $article_id ) );
        }
    ...
    function remove_submission( $sub_id ) {
        if ( $sub_id ) {
            ...
            $result = $this->query( $query, array( ( int ) $sub_id ) );
    ...
    function replace_submission( $title, $author_name, $topic_id, $use_image, $imgname, $imgsize, $imgtype, $imgdata, $heading, $body, $publish_date, $expire_date, $user, $sub_id, $image_x, $image_y, $type, $rating = 0, $isfloat = 'n' ) {
        ...
        $topic_name = $this->getOne( $query, array( ( int ) $topic_id ) );
        $size = strlen( $body );
        ...
        if ( $sub_id ) {
            ...
            $result = $this->query( $query, array( $title, $author_name, ( int ) $topic_id, $topic_name, ( int ) $size, $use_image, $isfloat, $imgname, $imgtype, ( int ) $imgsize, $this->mDb->db_byte_encode( $imgdata ), ( int ) $image_x, ( int ) $image_y, $heading, $body, ( int ) $publish_date, ( int ) $now, $user, $type, ( float ) $rating, ( int ) $sub_id ) );
        } else {
            // Insert the article
            ...
            $result = $this->query( $query, array( $title, $author_name, ( int ) $topic_id, $use_image, $imgname, ( int ) $imgsize, $imgtype, $this->mDb->db_byte_encode( $imgdata ), ( int ) $publish_date, ( int ) $now, $heading, $body, $hash, $user, 0, 0, 0, ( int ) $size, $topic_name, ( int ) $image_x, ( int ) $image_y, $type, ( float ) $rating, $isfloat ) );
        }
    ...
    function add_topic( $name, $imagename, $imagetype, $imagesize, $imagedata ) {
        ...
        $query = "insert into `" . BIT_DB_PREFIX . "tiki_topics`(`name`,`image_name`,`image_type`,`image_size`,`image_data`,`active`,`created`)
                     values(?,?,?,?,?,?,?)";
        $result = $this->query( $query, array( $name, $imagename, $imagetype, ( int ) $imagesize, $this->mDb->db_byte_encode( $imagedata ), 'y', ( int ) $now ) );

        $query = "select max(`topic_id`) from `" . BIT_DB_PREFIX . "tiki_topics` where `created`=? and `name`=?";
        $topic_id = $this->getOne( $query, array( ( int ) $now, $name ) );
        return $topic_id;
    }
    ...
    function remove_topic( $topic_id, $all = 0 ) {
        ...
        $result = $this->query( $query, array( $topic_id ) );
        ...
        if ( $all == 1 ) {
            ...
            $result = $this->query( $query, array( null, null, $topic_id ) );
        }
    ...
    function replace_topic_name( $topic_id, $name ) {
        $query = "update `" . BIT_DB_PREFIX . "tiki_topics` set `name` = ? where
			`topic_id` = ?";
        $result = $this->query( $query, array( $name, $topic_id ) );
    ...
    function activate_topic( $topic_id ) {
        ...
        $result = $this->query( $query, array( 'y', $topic_id ) );
    }

    function deactivate_topic( $topic_id ) {
        $query = "update `" . BIT_DB_PREFIX . "tiki_topics` set `active`=? where `topic_id`=?";

        $result = $this->query( $query, array( 'n', $topic_id ) );
    }
	...
	function get_topic_id($topic) {
    	...
    	$topic_id = $this->getOne($query, array($topic) );
	    return $topic_id;
	}
...
*/
        $result = $this->query( "insert into `" . BIT_DB_PREFIX . "tiki_article_types`(`type`) values(?)", array( $type ) );
	...
	function list_submissions($offset = 0, $maxRecords = -1, $sort_mode = 'publish_date_desc', $find = '', $date = '') {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_submissions` $mid order by ".$this->convert_sortmode($sort_mode);
	...
	function get_article($article_id) {
		...
		$result = $this->query($query,array((int)$article_id));
		if ($result->numRows()) {
		$res = $result->fetchRow();
		$res["image_data"] = $this->mDb->db_byte_decode( $res["image_data"] );
		$res["entrating"] = floor($res["rating"]);
	...
	function replace_article($title, $author_name, $topic_id, $use_image, $imgname, $imgsize, $imgtype, $imgdata, $heading, $body, $publish_date, $expire_date, $user, $article_id, $image_x, $image_y, $type, $rating = 0, $isfloat = 'n') {
		...
		$query = "select `name`  from `".BIT_DB_PREFIX."tiki_topics` where `topic_id` = ?";
		$topic_name = $this->getOne($query, array($topic_id) );
		$size = strlen($body);
		...
		if ($article_id) {
			...
			$result = $this->query($query, array( $title, $author_name, (int) $topic_id, $topic_name, (int) $size, $use_image, $imgname, $imgtype, (int) $imgsize, $this->mDb->db_byte_encode( $imgdata ), $isfloat, (int) $image_x, (int) $image_y, $heading, $body, (int) $publish_date, (int) $expire_date, (int) $now, $user, $type, (float) $rating, (int) $article_id ) );
		} else {
		// Fixed query. -rlpowell
		// Insert the article
		...
		$result = $this->query($query, array( $title, $author_name, (int) $topic_id, $use_image, $imgname, (int) $imgsize, $imgtype, $this->mDb->db_byte_encode( $imgdata ), (int) $publish_date, (int) $expire_date, (int) $now, $heading, $body, $hash, $user, 0, 0, 0, (int) $size, $topic_name, (int) $image_x, (int) $image_y, $type, (float) $rating, $isfloat));

		// Fixed query. -rlpowell
		$query2 = "select max(`article_id`) from `".BIT_DB_PREFIX."tiki_articles` where `created` = ? and `title`=? and `hash`=?";
		$article_id = $this->getOne($query2, array( (int) $now, $title, $hash ) );
		}
	...
	function get_topic_image($topic_id) {
		...
		$result = $this->query($query, array((int) $topic_id));
		$res = $result->fetchRow();
		$res['image_data'] = $this->mDb->db_byte_decode( $res['image_data'] );
		return $res;
	}

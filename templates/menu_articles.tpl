...
<ul>
	{if $gBitUser->hasPermission( 'p_articles_read' )}
		<li><a class="item" href="{$smarty.const.ARTICLES_PKG_URL}index.php">{biticon ipackage="icons" iname="go-home" iexplain="articles home" iforce=icon} {tr}Articles Home{/tr}</a></li>
		<li><a class="item" href="{$smarty.const.ARTICLES_PKG_URL}list.php">{biticon ipackage="icons" iname="format-justify-fill" iexplain="list articles" iforce=icon} {tr}List articles{/tr}</a></li>
		<li><a class="item" href="{$smarty.const.ARTICLES_PKG_URL}list_topics.php">{biticon ipackage="icons" iname="format-justify-fill" iexplain="list articles" iforce=icon} {tr}List topics{/tr}</a></li>
	{/if}
	{if $gBitSystem->isFeatureActive('articles_submissions')}
		{if $gBitUser->hasPermission( 'p_articles_submit ' ) ||
			...
			<li><a class="item" href="{$smarty.const.ARTICLES_PKG_URL}list.php?status_id={$smarty.const.ARTICLE_STATUS_PENDING}">{biticon ipackage="icons" iname="format-justify-fill" iexplain="view submissions" iforce=icon} {tr}List submissions{/tr}</a></li>
		{/if}
	{/if}
